...
# ---------------------------------------------------------------------------
# Auto-importar base del proyecto original (solo la primera vez que corre)
# ---------------------------------------------------------------------------
_imported = engine.bootstrap_from_legacy()


def _save_uploaded_to_temp(uploaded_file, suffix: str) -> str:
    """Guarda un archivo subido en disco por bloques para no saturar memoria RAM con archivos de hasta 7GB."""
    with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
        while True:
            chunk = uploaded_file.read(10 * 1024 * 1024)  # 10 MB por bloque
            if not chunk:
                break
            tmp.write(chunk)
        return tmp.name
...
if _imported > 0:
    st.success(
        f"📚 Se importaron automáticamente **{_imported} fragmentos** "
        "de la biblioteca de Física de la facu. ¡Ya podés analizar contenido!"
    )
...
with tab_video:
    ...
    if st.button("▶ Transcribir y comparar", disabled=video_file is None, type="primary", key="btn_video"):
        if not engine.references_summary():
            ...
            tmp_path = _save_uploaded_to_temp(video_file, suffix)
...
with tab_pdf:
    ...
    if st.button("🔍 Analizar PDF", disabled=pdf_file is None, type="primary", key="btn_pdf"):
        if not engine.references_summary():
            ...
            tmp_path = _save_uploaded_to_temp(pdf_file, ".pdf")
